package me.localserver.bungeesystem.database;
...
public class AsyncMySQL {
    ...
    public AsyncMySQL() {
        try {
            ...
            plugin = BungeeSystem.getInstance();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void query(PreparedStatement statement, Consumer<ResultSet> consumer) {
        executor.execute(() -> {
            ...
            BungeeSystem.getBungeeCord().getInstance().getScheduler().runAsync(plugin, () -> consumer.accept(result));
        });
    }
